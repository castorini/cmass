export const meta = {
  name: 'stage1-hop-clue-decomposition-and-grounding',
  description: 'Resumable Stage 1 Hop/Clue decomposition and Grounding for a chunk of BCP records: 1 agent/record decomposes -> match-centered ClimbMix retrieval -> Answerability check (PROJECTABLE/PARTIAL/NOT), and WRITES its own projections/bcp_<qid>.json (so budget cut-offs lose no completed work).',
  phases: [ { title: 'Projection', detail: '1 agent per record; self-contained inference-allowed projection; writes own output file' } ],
}
...
"",
"ACTION 1 - Read the task: " + taskPath(qid) + " (record_id, question, answer, provided_docs = original gold/qrel docs, as HINTS for the reasoning chain).",
"",
"ACTION 2 - Decompose the question into the minimal HOPS (atomic facts) on the path from the question's constraints to the answer (use provided_docs to identify the chain; aim 4-6).",
"",
"ACTION 3 - For each hop, retrieve from ClimbMix and judge support (INFERENCE ALLOWED - a hop counts if stated in OR soundly inferable from corpus docs: multi-doc synthesis, unit/temporal/arithmetic conversion, narrative deduction, entity linking). Previews are CENTERED ON THE QUERY MATCH:",
...
"",
"ACTION 4 - Decide the VERDICT using the UNIQUENESS test (the answer may itself be reasoned/computed, need not be verbatim):",
...
"",
"ACTION 5 - WRITE your result (Write tool) to: " + outPath(qid),
...
"",
"ACTION 6 - return StructuredOutput: record_id, verdict, n_hops, n_supported_hops, wrote_file=true, note (one line).",
